...
let NodeModulesUpdateIndicator = Jakets.MakeRelativeToWorkingDir("node_modules/.node_modules_updated");
let JakefileDependencies = Jakets.MakeRelativeToWorkingDir("Jakefile.dep.json");
...
let NpmUpdateTask = Jakets.FileTask(
  ...
  , async function () {
    ...
    return Jakets.ExecAsync(`npm install && npm update && touch ${this.GetName()}`);
  }
);
